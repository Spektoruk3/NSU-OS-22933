...
int main(int argc,char* argv[]) {
	...
	offsetBeforeString.data[2] = 0;
	offsetBeforeString.size = 3;
	...
	while(1) {
		...
		lseek(file_descriptor, offsetBeforeString.data[request], SEEK_SET);
		int countOfElement = offsetBeforeString.data[request + 1] - offsetBeforeString.data[request];
		printf("String:");
